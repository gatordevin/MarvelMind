...
class MarvelmindHedge (Thread):
    ...
    def IMUData(self):
        return list(self.valuesImuData)
    def position(self):
                            ...
                            try:
                                if (len(bufferList) > pktHdrOffset + 4 + msgLen + 2):
                                    ...
                                    #         self.recieveUltrasoundRawDataCallback()
                                        ...
                                        qx = qx/10000
                                        qz = qz/10000
                                        value = [x / 1000.0, y / 1000.0, z / 1000.0, self.quaternion_to_euler_angle(qw, qx, qy, qz)[0], self.quaternion_to_euler_angle(qw, qx, qy, qz)[1], self.quaternion_to_euler_angle(qw, qx, qy, qz)[2]]
                                        self.valuesImuData.append(value)
                                        #print('X:', x, '  Y:', y, '  Z:', z,'  X:', self.quaternion_to_euler_angle(qw, qx, qy, qz)[0],'  Y:', self.quaternion_to_euler_angle(qw, qx, qy, qz)[1],'  Z:', self.quaternion_to_euler_angle(qw, qx, qy, qz)[2])
